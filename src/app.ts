...
export function createApp(): Hono<AppBindings> {
  ...
  app.get('/health', (context) => context.json({
    status: 'ok', service: 'spendsnap-api', buildSha: context.env.BUILD_SHA ?? 'unknown',
    timestamp: new Date().toISOString(), requestId: context.get('requestId'),
  }));
  ...
  app.onError((error, context) => {
    ...
    return context.json({
      error: {
        ...
        ...(normalized.details ? { details: normalized.details } : {}), requestId,
      },
